...
public class PokerHub extends Hub {
	...
	protected void messageReceived(int ClientID, Object message) {
		...
		if (message instanceof Action) {
			...
			case Draw:
				resetOutput();
				int maxCards = HubGamePlay.getRule().getPlayerCardsMax();
				int playerCards =  HubGamePlay.getRule().GetPlayerNumberOfCards();
				int drawCount = 2;
				if(maxCards >= playerCards & playerCards != 0){
	
					for(eDrawCount i : eDrawCount.values()){
						if(drawCount == i.getDrawNo()){
							HubGamePlay.setDrawCnt(i);
						}
					}
				...
				if(maxCards == playerCards){
					int highScore = 0;
					Player winner = new Player();
					for(Player player : HubGamePlay.getGamePlayers().values()){
						if(HubGamePlay.getPlayerHand(player.getPlayerID()).getHandScore().getHandStrength() > highScore){
							//This will return the player with the highest handStrength
							highScore = HubGamePlay.getPlayerHand(player.getPlayerID()).getHandScore().getHandStrength();
							winner = player;
						}
					}
					GamePlayPlayerHand HubGamePlayPlayerHand = new GamePlayPlayerHand(HubGamePlay, winner);
					HubGamePlayPlayerHand.setWinningPlayer(winner);
				}
				break;
			}
		}
